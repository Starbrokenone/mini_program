  ...
  bindSearchInput: function (e) {
    ...
    if (e.detail.cursor >= this.data.cursor) {
      ...
      if (inputData) {
        ...
        //为search赋值
        for (var i = 0; i <= 2; ++i) {
          this.data.buildlData.push(app.globalData.full_map[i])
        }
        //console.log(this.data.buildlData)
        console.log(app.globalData.pageId)
          let searchdata = this.data.buildlData[app.globalData.pageId];
          let z = 0, x = 100;
          for (var b in searchdata) {
            for (var i in searchdata[b].data) {
              if (searchdata[b].data[i].name.indexOf(inputData) != -1 || (searchdata[b].data[i].floor && searchdata[b].data[i].floor.indexOf(inputData) != -1)) {
                ...
                build.pid = app.globalData.pageId;
                build.tid = b;
                build.bid = i;
                z = z + 1;
                build.index = z;
          ...
          //去重
          for (var m = 0; m < showData.length - 1; ++m) {
            var cur = showData[m];
            for (var n = m + 1; n < showData.length; ++n) {
              if (cur.latitude === showData[n].latitude && cur.name == showData[n].name) {
                showData.splice(n, 1);//出现数组塌
                n--;
              }
            }
          }
